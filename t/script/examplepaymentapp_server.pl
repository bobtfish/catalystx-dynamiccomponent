...
my $help              = 0;
my $host              = undef;
my $port              = $ENV{EXAMPLEPAYMENTAPP_PORT} || $ENV{CATALYST_PORT} || 3000;
my $keepalive         = 0;
my $restart           = $ENV{EXAMPLEPAYMENTAPP_RELOAD} || $ENV{CATALYST_RELOAD} || 0;
my $background        = 0;
my $pidfile           = undef;
...
my $runner = sub {
    ...
    require ExamplePaymentApp;

    ExamplePaymentApp->run(
        $port, $host,
        {
            argv       => \@argv,
            'fork'     => $fork,
...
examplepaymentapp_server.pl - Catalyst Testserver

=head1 SYNOPSIS

examplepaymentapp_server.pl [options]
